...
# Quick uninstall flag support
if [ "$1" = "--uninstall" ] || [ "$1" = "-u" ] || [ "$1" = "uninstall" ]; then
    if [ -x "/usr/local/bin/ctrlmcp" ]; then
        exec /usr/local/bin/ctrlmcp uninstall "$2"
    else
        echo -e "${YELLOW}[*] CTRLMCP CLI not found in /usr/local/bin/ctrlmcp. Performing direct cleanup...${NC}"
        ...
        rm -rf /opt/ctrlmcp /etc/ctrlmcp /var/log/ctrlmcp_audit.log /etc/profile.d/ctrlmcp.sh
        sed -i '/ctrlmcp/d' /etc/bash.bashrc 2>/dev/null || true
        rm -f /usr/local/bin/ctrlmcp /usr/local/bin/CtrlMCP /usr/local/bin/CTRLMCP
        echo -e "${GREEN}✓ All CTRLMCP components removed completely from root.${NC}"
        exit 0
    fi
fi
